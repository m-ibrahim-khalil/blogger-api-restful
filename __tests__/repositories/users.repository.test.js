...
describe('Testing Users Repository: ', () => {
  ...
  describe('Testing updatePasswordByUsername function: ', () => {
    it('should update a user by username and return 1', async () => {
      const username = 'testuser';
      const password = 'testpassword';
      jest.spyOn(User, 'update').mockResolvedValueOnce([1]);

      const user = await UsersRepository.updatePasswordByUsername(username, password);
    ...
    it('should throw an error if there is an error in the database query', async () => {
      ...
      await expect(
        UsersRepository.updatePasswordByUsername(username, password)
      ).rejects.toThrow(expectedError);
    });
  });
  ...
  describe('Testing deleteUser function: ', () => {
    ...
    it('should throw an error if there is an error in the database query', async () => {
      ...
      await expect(UsersRepository.deleteByUsername(username)).rejects.toThrow(
        expectedError
      );
    });
  });
