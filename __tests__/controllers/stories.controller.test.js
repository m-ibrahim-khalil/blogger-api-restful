...
const { storiesDB } = require('../__mocks__/testDB');
...
describe('Testing StoriesControler', () => {
  ...
  describe('createStory method', () => {
    it('should return a success response when create a story', async () => {
      ...
      const coverImageURL = 'http://test.com';
      const req = {
        username,
        body: {
          title,
  ...
  describe('updateById method', () => {
    it('should return a success response when updating a story', async () => {
      ...
      jest
        .spyOn(StoriesService, 'updateById')
        .mockResolvedValueOnce(expectedResponse);
      jest
        .spyOn(ContentNegotiation.prototype, 'sendResponse')
        .mockResolvedValueOnce(expectedResponse);

      const response = await StoriesControler.updateById(req, res, next);

      expect(StoriesService.updateById).toHaveBeenCalledWith(
        id,
        title,
        description,
        coverImageURL
    ...
    it('should throw a BadRequestError when id parameter is missing', async () => {
      ...
      await StoriesControler.updateById(req, res, next);
      expect(next).toHaveBeenCalledWith(expectedError);
    });
  ...
  describe('deleteById', () => {
    it('should return a success response when deleting a story', async () => {
      ...
      jest
        .spyOn(StoriesService, 'deleteById')
        .mockResolvedValueOnce(expectedResponse);
      jest
        .spyOn(ContentNegotiation.prototype, 'sendResponse')
        .mockResolvedValueOnce(expectedResponse);

      const response = await StoriesControler.deleteById(req, res, next);

      expect(StoriesService.deleteById).toHaveBeenCalledWith(id);
    ...
    it('should throw a BadRequestError when id parameter is missing', async () => {
      ...
      await StoriesControler.deleteById(req, res, next);
      expect(next).toHaveBeenCalledWith(expectedError);
    });
  });
});
